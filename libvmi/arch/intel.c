...
/* page table */
static inline
uint32_t pte_index_nopae (vmi_instance_t instance, uint32_t address)
{
    return (((address) >> 12) & 0x3FFUL) * sizeof(uint32_t);
}

static inline
uint32_t pte_index_pae (vmi_instance_t instance, uint32_t address)
{
...
{
    uint32_t value;
    *pte_entry = ptba_base_nopae(pgd) + pte_index_nopae(instance, vaddr);
...
{
    uint64_t value;
    *pte_entry = ptba_base_pae(pgd) + pte_index_pae(instance, vaddr);
...
static inline
uint32_t get_large_paddr_nopae (vmi_instance_t instance, uint32_t vaddr,
        uint32_t pgd_entry)
{
...
static inline
uint32_t get_large_paddr_pae (vmi_instance_t instance, uint32_t vaddr,
        uint32_t pgd_entry)
{
    ...
    page_info_t *info)
{
    ...
    if (ENTRY_PRESENT(vmi->os_type, pgd)) {
        ...
        if (PAGE_SIZE_FLAG(pgd)) {
            info->paddr = get_large_paddr_nopae(vmi, vaddr, pgd);
    ...
    page_info_t *info)
{
    ...
    if (ENTRY_PRESENT(vmi->os_type, pgd)) {
        info->l2_v = pgd;
        if (PAGE_SIZE_FLAG(pgd)) {
            info->paddr = get_large_paddr_pae(vmi, vaddr, pgd);
